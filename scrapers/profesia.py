"""
Scraper pro Profesia.cz
Vyhledává pracovní nabídky na Profesia.cz
"""
...
class ProfesiaScraper(BaseScraper):
    ...
    def __init__(self):
        """Inicializace Profesia.cz scraperu"""
        super().__init__("profesia")
        self.base_url = config.TARGET_PORTALS["profesia"]["base_url"]

    def _build_search_url(self, keyword: str) -> str:
        """Sestaví search URL pro Profesia.cz"""
        encoded_keyword = urllib.parse.quote_plus(keyword)
        return f"{self.base_url}/praca/?search_anywhere={encoded_keyword}"

    def scrape(self, keywords: List[str]) -> List[Dict]:
        """Scrapuje Profesia.cz pro zadaná klíčová slova"""
        all_jobs = []
        self.logger.info(f"Začínám scraping Profesia.cz s {len(keywords)} klíčovými slovy")
        ...
        self.logger.info(f"Celkem nalezeno {len(all_jobs)} nabídek na Profesia.cz")
        return all_jobs
    ...
    def _extract_jobs_from_page(self, soup, keyword: str) -> List[Dict]:
        ...
        # UKÁZKOVÉ selektory
        job_listings = soup.find_all('div', class_='list-row')

        if not job_listings:
            job_listings = soup.find_all('article', class_='offer-item')
    ...
    def _extract_job_details(self, job_element) -> Dict:
        ...
        try:
            # Název pozice
            nazev_elem = job_element.find('h2', class_='title')
            if not nazev_elem:
            ...
            # Společnost
            spolecnost_elem = job_element.find('span', class_='employer')
            spolecnost = spolecnost_elem.get_text(strip=True) if spolecnost_elem else "N/A"

            # Lokace
            lokace_elem = job_element.find('span', class_='place')
            lokace = lokace_elem.get_text(strip=True) if lokace_elem else "N/A"

            # Mzda
            mzda_elem = job_element.find('span', class_='wages')
